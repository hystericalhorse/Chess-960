...
namespace Chess
{
    ...
    ***************************************************************************/
    partial class Board : Form
    {
        ...
        ***************************************************************************/
        public enum Mode { Chess, Chess960 }
        public Mode _mode;
        public Board(Mode mode = Mode.Chess)
        {
        ...
        ***************************************************************************/
        ...
        private void Initialize960Board()
        {
            ...
            {
                PlacePiece(new Pawn(this, Color.White), new Position(6, column));
                PlacePiece(new Pawn(this, Color.Black), new Position(1, column));
            }

            SetPieces960(7, Color.White);
            SetPieces960(0, Color.Black);
        }

        private void SetPieces960(int row, Color pieceColor)
        {
            int[] columns = new int[8] { 0, 0, 0, 0, 0, 0, 0, 0 };
            ...
            {
                col = rand.Next(0, 8);
                isValidSet = col % 2 == 0 && columns[col] != 1;
            }
            while (!isValidSet);

            PlacePiece(new Bishop(this, pieceColor), new Position(row, col));
            columns[col] = 1;
            ...
            {
                col = rand.Next(0, 8);
                isValidSet = col % 2 == 1 && columns[col] != 1;
            }
            while (!isValidSet);
            ...
            {
                col = rand.Next(0, 8);
                isValidSet = columns[col] != 1;
            }
            while (!isValidSet);

            PlacePiece(new Queen(this, pieceColor), new Position(row, col));
            columns[col] = 1;
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                PlacePiece(new Knight(this, pieceColor), new Position(row, col));
                columns[col] = 1;
            }

            // Evalutate Remaining Spaces
            int piecesLeft = 0;
            for (col = 0; col < 8; col++)
            {
                if (columns[col] == 0)
                {
                    if (piecesLeft == 1)
                    {
                        PlacePiece(new King(this, pieceColor), new Position(row, col));
                        piecesLeft++;
                    }
                    else
                    {
                        PlacePiece(new Rook(this, pieceColor), new Position(row, col));
                        piecesLeft++;
                    }
                }
            }
