        ...
        ***************************************************************************/
        public override bool[,] PossibleMoves()
        {
            ...
            if (NumberOfMoves == 0 && !ReceivedCheck)
            {
                ...
                Position positionRook1 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, 3));
                if (CheckRookAvailableForCastling(positionRook1))
                {
                    // Check if the positions between the King and the Rook are empty
                    Position position1 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, 1));
                    Position position2 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, 2));
                    if (ChessBoard.GetPiece(position1) == null &&
                        ChessBoard.GetPiece(position2) == null)
                    {
                        matrix[CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, 2)] = true;
                    }
                }

                // Queenside castling special move
                Position positionRook2 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, -4));
                if (CheckRookAvailableForCastling(positionRook2))
                {
                    // Check if the positions between the King and the Rook are empty
                    Position position1 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, -1));
                    Position position2 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, -2));
                    Position position3 = new Position(CurrentPosition.Row, TryOffsetPositionWithinBounds(CurrentPosition.Column, -3));
                    if (ChessBoard.GetPiece(position1) == null &&
                        ChessBoard.GetPiece(position2) == null &&
        ...
        public int TryOffsetPositionWithinBounds(int position, int offset, int minInclusive = 0, int maxInclusive = 7)
        {
